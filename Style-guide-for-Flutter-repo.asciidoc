...
Avoid using class names with `iOS` when possible, since that would capitalize as `IOS` (due to the https://dart.dev/guides/language/effective-dart/style#do-capitalize-acronyms-and-abbreviations-longer-than-two-letters-like-words[two-letter abbreviation exception] to normal Dart naming rules), which is jarring; use alternatives like "Cupertino" or "UIKit" instead when possible. (Previous versions of this guide incorrectly indicated that `Ios` was the correct capitalization when necessary; this form should not be used in new code.)
